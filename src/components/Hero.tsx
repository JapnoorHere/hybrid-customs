...
export default function Hero() {
  ...
  const slides = [
    "/images/main_bg.jpg",
    "/images/fortuner-hero.webp",
    "/images/suv-hero.png",
    "/images/interior-home.png",
    "/images/thar-home.jpg",
    "/images/ceramic-hero.avif",
    "/images/mercedes-hero.png",
  ];
  ...
  return (
    <section className="relative min-h-[105vh] flex items-center overflow-hidden bg-black">
      ...
      {/* ── Slide Indicators ── */}
      <div className="absolute left-8 lg:left-10 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-3">
        {slides.map((_, i) => (
          <button
            key={i}
            onClick={() => setCurrentIndex(i)}
            className={`w-1 h-8 transition-all duration-500 rounded-full ${i === currentIndex ? "bg-primary" : "bg-white/10 hover:bg-white/30"}`}
          />
        ))}
      </div>
        ...
        <div className="max-w-2xl flex flex-col items-center lg:items-start">
          ...
          >
            ...
            <span className="block text-white">
              Your <span className="text-gradient">Machine.</span>
            </span>
          </h1>
          ...
          >
            Performance body mods, custom paint jobs, car care, and signature illumination — engineered for enthusiasts.
          </p>
          ...
          >
            <a
              href="#services"
              className="btn-lift group relative px-8 py-3.5 bg-primary text-black font-bold uppercase tracking-[0.12em] text-xs hover:bg-white transition-all duration-300 text-center overflow-hidden"
            >
              Explore Services
            </a>
            <a
              href="https://wa.me/919988229191"
              className="px-8 py-3.5 border border-white/15 text-white font-bold uppercase tracking-[0.12em] text-xs hover:border-primary/60 hover:text-primary transition-all duration-300 backdrop-blur-sm bg-white/[0.03] text-center hover:translate-y-[-2px] hover:shadow-lg"
            >
              Order Online
            </a>
          </div>
          ...
          {/* Stats row */}
          <div
            className={`flex gap-10 mt-14 pt-8 border-t border-white/[0.06] transition-all duration-1000 delay-[900ms] ${
              mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
            }`}
          >
            {[
              { value: `${new Date().getFullYear() - 2009}+`, label: "Years" },
              { value: "100%", label: "Passion" },
            ].map((stat) => (
              <div key={stat.label} className="stat-item cursor-default text-left">
                <div className="text-2xl md:text-3xl font-black text-white tracking-tight">
                  {stat.value}
                </div>
                <div className="text-[9px] uppercase tracking-[0.2em] text-white/40 mt-1 font-medium">
                  {stat.label}
                </div>
              </div>
            ))}
